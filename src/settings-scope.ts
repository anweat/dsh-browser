...
function legacyRegister(settings: unknown): LegacySettingsRegister['register'] | undefined {
  if (settings === null || typeof settings !== 'object') return undefined
  const candidate = (settings as { register?: unknown }).register
  return typeof candidate === 'function'
    ? (candidate as LegacySettingsRegister['register']).bind(settings)
    : undefined
}
 ...
 */
export function resolveSettingsScope<TConfig>(
  settings: unknown,
  namespace: string,
  schema: unknown,
  entryConfig: TConfig,
): SettingsResolution<TConfig> {
  const register = legacyRegister(settings)
  if (register === undefined) return { get: () => entryConfig, mode: 'entry' }
  ...
  return { get: () => scope.get() as TConfig, mode: 'scope' }
}
