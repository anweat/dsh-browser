...
const header = (name, description) => `// ==UserScript==
// @name ${name}
...
export function matchUserscriptPattern(pattern, targetUrl) {
    const parsed = pattern.match(/^(\*|https?|file|ftp):\/\/([^/]+)(\/.*)$/i);
    if (!parsed)
        return false;
    const url = new URL(targetUrl);
    if (parsed[1] === '*' ? !/^https?:$/.test(url.protocol) : url.protocol !== parsed[1].toLowerCase() + ':')
        return false;
...
function capabilitiesOf(source) {
    const capabilities = new Set(['dom-read']);
    if (/\b(fetch|XMLHttpRequest|WebSocket|sendBeacon)\b/.test(source))
        capabilities.add('network');
    if (/\b(localStorage|sessionStorage|indexedDB|document\.cookie)\b/.test(source))
        capabilities.add('storage');
    if (/\.click\s*\(|dispatchEvent\s*\(|\.submit\s*\(|\blocation\s*=|history\.(pushState|replaceState)/.test(source))
        capabilities.add('interaction');
    if (/\b(remove|append|prepend|replaceWith|insertAdjacentHTML)\s*\(|\.innerHTML\s*=|\.textContent\s*=/.test(source))
        capabilities.add('dom-write');
    return [...capabilities];
}
export function validateUserscript(source, targetUrl) {
    const bytes = Buffer.byteLength(source, 'utf8');
    const sha256 = crypto.createHash('sha256').update(source, 'utf8').digest('hex');
    const parsed = metadataOf(source);
    const errors = [];
    const warnings = [];
    if (bytes < 1 || bytes > 64 * 1024)
        errors.push('source must contain between 1 and 65536 UTF-8 bytes');
    if (!source.includes('// ==UserScript==') || !source.includes('// ==/UserScript=='))
        errors.push('userscript metadata block is required');
    if (parsed.matches.length < 1)
        errors.push('at least one @match is required');
    if (parsed.matches.some(pattern => !/^(\*|https?):\/\//i.test(pattern)))
        errors.push('only HTTP(S) @match patterns are supported');
    if (parsed.requires.length > 0)
        errors.push('@require is not supported; external code must be supplied inline for approval');
    const invalidGrants = parsed.grants.filter(grant => grant !== 'none');
    if (invalidGrants.length > 0)
        errors.push('unsupported @grant values: ' + invalidGrants.join(', '));
    if (parsed.grants.length === 0)
        warnings.push('missing @grant; execution still provides no GM_* APIs');
    if (targetUrl !== undefined) {
        ...
        if (!matched)
            errors.push('target URL is outside the userscript @match scope');
    }
    return {
...
export function builtinScript(id) {
    const script = BUILTIN_SCRIPTS.find(candidate => candidate.id === id);
    if (!script)
        throw new Error('unknown built-in script: ' + id);
    return script;
}
export async function executeUserscript(page, source, maxResultChars = 100_000, inputs = {}) {
    ...
    if (truncated)
        resultJson = resultJson.slice(0, maxResultChars) + '…';
