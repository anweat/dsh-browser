...
`;
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										selected ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											...
											className: styles.secondary,
											disabled: state.busy,
											onClick: () => props.testAutomationAsset(selected.id),
		...
		const zh = {
			...
			assetLibrary: "可复用自动化资产",
			assetLibraryHint: "候选先提示是否总结；草稿测试后再手动激活。源码仅在点选编辑时读取。",
		...
		const en = {
			...
			assetLibrary: "Reusable automation assets",
			assetLibraryHint: "Candidates ask before summarization; drafts are tested before manual activation. Source loads only when selected.",
			...
			assetInvalid: "Invalid JSON or asset fields.",
		...
		var AutomationAssetsController = class {
			...
			inject() {
				return {
					...
					testAutomationAsset: (id) => this.mutate("test", { id }),
