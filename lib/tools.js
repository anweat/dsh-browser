    ...
    register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (!asset || asset.status !== 'active')
                throw new Error('active automation asset not found');
            ...
            if (missingInputs.length)
                throw new Error('missing declared automation inputs: ' + missingInputs.join(', '));
            if (extraInputs.length)
                throw new Error('undeclared automation inputs: ' + extraInputs.join(', '));
            let value;
            try {
                if (asset.kind === 'recipe')
                    value = await service.recipe(materializeRecipe(asset.recipe ?? [], inputs), { url: args.url, signal: exec.signal, ...args.authProfile ? { authProfile: args.authProfile } : {}, ...args.rulePack ? { rulePack: args.rulePack } : {} });
                else
                    value = await service.runUserscript(args.url, asset.source ?? '', { signal: exec.signal, inputs, ...args.authProfile ? { authProfile: args.authProfile } : {}, ...args.rulePack ? { rulePack: args.rulePack } : {} });
                assets.noteRun(asset.id, true);
            }
            catch (error) {
                assets.noteRun(asset.id, false);
                throw error;
            }
