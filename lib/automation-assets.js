...
export class AutomationAssetStore {
    ...
    search(query, domain, status = 'active', kind) {
        if (!query.trim())
            throw new Error('automation search requires explicit keywords');
        if (!['draft', 'active', 'archived', 'all'].includes(status))
            throw new Error('invalid automation search status');
    ...
    noteTestResult(id, ok, url) {
        ...
        if (asset.status !== 'draft')
            throw new Error('only draft automation assets can record test results');
        const domain = safeDomain(url);
