...
import { AuthProfileStore, hostAllowed } from "./auth-profiles.js";
...
import { installDialogGuard } from "./dialogs.js";
...
export class BrowserService {
    ...
    async transientContext(url, opts = {}) {
        ...
        if (profile && stateOptions.storageState) {
            ...
            scopedState = {
                cookies: (state.cookies ?? []).filter((cookie) => hostAllowed(cookie.domain.replace(/^\./, ''), profile.allowedDomains)),
                origins: (state.origins ?? []).filter((origin) => hostAllowed(new URL(origin.origin).hostname, profile.allowedDomains)),
            };
        }
    ...
    async runScript(url, source, opts = {}) {
        ...
        try {
            ...
            if (!finalValidation.valid)
                throw new Error('userscript redirected outside its allowed match: ' + finalValidation.errors.join('; '));
