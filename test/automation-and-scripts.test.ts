...
test('real Playwright runtime executes built-ins, recipes, and a scoped userscript', async () => {
  ...
  try {
    ...
    const uploaded = await service.setFiles('#upload', [uploadPath])
    assert.deepEqual(uploaded.files, ['fixture.txt'])
    assert.match(uploaded.text, /fixture\.txt/)
    await assert.rejects(() => service.setFiles('#upload', ['relative.txt']), /absolute file paths/)

    const evaluated = await service.evaluate(`(() => {
      document.cookie = 'visible_cookie=fixture; SameSite=Lax'
      localStorage.setItem('fixture', 'stored')
      return { title: document.title, cookie: document.cookie, stored: localStorage.getItem('fixture') }
    })()`)
    assert.deepEqual(JSON.parse(evaluated.resultJson), { title: 'Fixture', cookie: 'visible_cookie=fixture', stored: 'stored' })
    assert.deepEqual(evaluated.capabilities, ['dom', 'page-javascript', 'page-network', 'page-storage'])
